...
def merge_and_summarize(commits_df: pd.DataFrame, issues_df: pd.DataFrame) -> None:
    ...
    closed_issues_df = issues[issues['state'] == 'closed'].copy()
    
    closed_issues_with_dates = closed_issues_df[
            closed_issues_df['created_at'].notna() & 
            closed_issues_df['closed_at'].notna()
        ].copy()
        
    if not closed_issues_with_dates.empty:
        closed_issues_with_dates['open_duration'] = (
            closed_issues_with_dates['closed_at'] - closed_issues_with_dates['created_at']
        ).dt.total_seconds() / (24 * 3600)  # Convert to days
        
        avg_duration = closed_issues_with_dates['open_duration'].mean()
        print(f"Average Open Duration for Closed Issues: {avg_duration:.1f} days")
    else:
        print("Average Open Duration for Closed Issues: No closed issues with valid dates")
    print()
